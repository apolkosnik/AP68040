	...
	if (!nreset) begin
		...
		case (state)
			...
			S_BF_MEM0: begin : bf_mem0
				...
				// OPENSTEP's WindowServer dies on BFTST 3(A1){6:2} at the
				// last mapped byte, where the speculative longword reaches
				// three bytes into the next page.  The span is already
				// computed above and the write path already cases on it.
